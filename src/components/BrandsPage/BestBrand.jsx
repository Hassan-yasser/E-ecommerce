...
export default function BestBrand() {
  ...
  let BrandProducts = data?.data?.data?.filter((product)=>product.brand.name == BrandName)
  console.log(BrandProducts);
  return <>
  {BrandProducts?.length > 0 ?  <>
    <Helmet>
  <title>Fresh {BrandName}</title>
  </Helmet>
  <section className='grid grid-cols-12 gap-5'>
     <div className='col-span-12 text-center flex justify-center'><h1 className='text-2xl font-bold pb-1 w-fit relative before:absolute before:bottom-0 before:w-1/2 before:left-1/2 before:-translate-x-1/2 before:h-[4px] before:bg-vip rounded-md'>{BrandName} Products</h1></div>
